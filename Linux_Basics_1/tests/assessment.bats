...
EOF
  ...
  run_command find . -iname 'file-*' -type f
  ...
  run_command find . -iname 'file-*' -type d
...
@test "format block device '/dev/loop0' as ext4 and mount to '/mnt/virtual'" {
  ...
  run_command sudo mount -t ext4 /dev/loop0 /mnt/virtual
...
@test "list processes running for user 'tester'" {
  case1="$(echo "When." | tr "hW" "sp" | tr "en" " -" | tr "." "U")"
  case2="$(echo "When." | tr "hW" "sp" | tr "en" " -" | tr "." "u")"
  # Write a command below to list all processes for user 'tester'
  command="ps -u tester"

  (echo "$command" | grep "$case1 tester") || (echo "$command" | grep "$case2 tester")
}
...
EOF
  ...
  run_command echo SOME MESSAGE > output-file
  [ "$status" -eq 0 ]
